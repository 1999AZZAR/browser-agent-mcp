...
The skill is the entry point. The power is in `mcp__browser-agent__*` tools — 29 tools covering the full Playwright API over CDP. Always call MCP tools directly; this skill maps task types to exact calls.
...
| Run JS in page | `browser_evaluate(script)` | — |
